...
--  PULPIT AS 'Кафедра',
--  PROFESSION AS 'Специальность',
--  SUBJECT.SUBJECT AS 'Дисциплина',
--  NAME AS 'Студент',
--CASE 
--WHEN (PROGRESS.NOTE = 6) THEN 'шест'
--WHEN (PROGRESS.NOTE = 7) THEN 'сем'
--WHEN (PROGRESS.NOTE = 8) THEN 'восем'
--END [Оценки]
--FROM 
--  PROGRESS 
--INNER JOIN  
--  STUDENT ON PROGRESS.IDSTUDENT=STUDENT.IDGROUP
--INNER JOIN
--  SUBJECT ON PROGRESS.SUBJECT=SUBJECT.SUBJECT 
--INNER JOIN 
--  GROUPS ON STUDENT.IDGROUP = GROUPS.
...
SELECT 
  ISNULL(TEACHER.TEACHER_NAME,'***')[Преподаватели] ,
